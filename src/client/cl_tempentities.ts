...
import { CL_BlasterParticles } from "./cl_effects"
import { CL_ParticleEffect } from "./cl_particles"
...
export function CL_ParseTEnt(msg: QReadbuf) {
	// int type;
	// vec3_t pos, pos2, dir;
	// explosion_t *ex;
	// int cnt;
	...
	switch (type) {
		case SHARED.temp_event_t.TE_BLOOD: /* bullet hitting flesh */
			pos = msg.ReadPos()
			dir = msg.ReadDir()
			CL_ParticleEffect(pos, dir, 0xe8, 60);
			break;

		case SHARED.temp_event_t.TE_GUNSHOT: /* bullet hitting wall */
		case SHARED.temp_event_t.TE_SPARKS:
		case SHARED.temp_event_t.TE_BULLET_SPARKS:
			pos = msg.ReadPos()
			dir = msg.ReadDir()

			if (type == SHARED.temp_event_t.TE_GUNSHOT) {
		...
		case SHARED.temp_event_t.TE_SHOTGUN: /* bullet hitting wall */
			pos = msg.ReadPos()
			dir = msg.ReadDir()
		...
		case SHARED.temp_event_t.TE_BLASTER: /* blaster hitting wall */
			pos = msg.ReadPos()
			dir = msg.ReadDir()
			CL_BlasterParticles(pos, dir);
		...
		default:
			Com_Error(SHARED.ERR_DROP, `CL_ParseTEnt: bad type ${type}`);
	}
}
